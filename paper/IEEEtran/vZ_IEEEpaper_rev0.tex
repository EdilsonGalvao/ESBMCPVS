...
\color{red} neste parágrafo acima, precisamos ter cuidado, pois o HOMER Pro faz sim simulação com otimização embutida e, portanto, apresenta o projeto ótimo. A diferença está no espaço de estados avaliados, onde a simulação nunca cobre todas as possibilidades, enquanto nossa técnica faz uma busca mais ampla. Retirando do artigo VSTTE "However, these tools are based on simulation; they have the drawback of incomplete coverage of design-space since verification of all possible combinations, and potential failures of a system are not feasible [10]."
...
\color{red} aqui eu creio que há um problema de entendimento. Contraexemplo, pra mim, só ocorre se há um FAIL na verificação. Caso contrário, ele é SUCCESS a ferramenta encerra sem achar uma solução factível/ótima.
Creio que, a partir do próximo parágrafo, antes de apenas citar o base paper (VSTTE), ou usar o based on, acredito que deve ser delimitado o que foi feito previamente para que a audiência entenda; e mais ainda seja acrescido os trabalhos relacionados, para mostrar o estado-da-arte no tema. Esse parágrafo deve aparecer aqui. Acredito que uma leitura no parágrafo que inicia com "Optimization of PV systems is not a recent topic; since the 1990s,..." no artigo VSTTE vai ajudar
...
For \textbf{Input} data, we can consider weather data, price information about each equipment, design requirements, load curve, power demand, and design assumptions. This information is converted programmatically into a matrix of data and exposed as global content to the software. For \textbf{Output} data, the major information denotes that renewable energy systems can be combined (price and system hardware composition), when it occurs, the answer of the system need be SAT. Otherwise, the answer of the system is UNSAT. \color{red} aqui o entendimento de SAT e UNSAT (ou SUCCESS) está trocado
%
\begin{figure}[h]
...
Often, a very large number of states will cause classic SMT solvers to demand more time than is acceptable, as described in ~\ref{ObjectivesAndSetup}, this would override the EG01 objective, since it would not be possible to verify that all tools compared here produce correct answers. We will then use two approaches that will guarantee the demonstration of correctness and the robustness of the proposed system. The first, reduces the state space by ensuring that all solvers described in the previous paragraph will have an answer to the problem in a timely manner. Finally, to ensure the robustness of the proposed tool, we will significantly increase the state space so that we can validate the tool and face market programs. Both approaches must achieve the same result, since the reduced state space (the case study resul) is a subset of the total analyzed space, and both converge to the same solution.\color{black}
...
\color{blue} Table~\ref{Tab:Tcr} condenses the result of both approaches described in section~\ref{sec:SolversandStateSpace}. The table is divided into four important sets of columns, read from left to right: Column 01 describes all test cases with their respective specifications. Columns 02, 03 and 04 refer to the first approach that proves the correctness of the system. Columns 05, 06 and 07 refer to the second approach, which proves the robustness of the tool used for commercial analysis and finally the last column refers to the simulation of the photovoltaic system using Homer Pro.

%In each result analysis there is (SAT) when a solution cannot be found or (UNSAT) when a solution is found.

\textbf{First approach:} In view of the state explosion problem mentioned in section~\ref{sec:SolversandStateSpace}, which demands enormous time and computational power from the ESBMC tool and the CPAChecker tool, it was necessary to choose the smallest possible amount of data that would satisfy the computational calculations in order to find the better configuration of electrical equipment for the photovoltaic system. In order to reach the goal of 70 different devices described in section~\ref{sec:SynthesizingOptimalSolarPhotovoltaicSystems}, we need to process a smaller set of data in this approach, totaling only 10 devices divided into 4 different categories and represented in Figure~\ref{fig:blockdiagram}. This change implies reducing the sample processing space by approximately 3890 times, consequently, the processing time will be reduced.

Analyzing column number 2 of table~\ref{Tab:Tcr}, it is possible to observe that all test cases returned SAT as a response, this implies that there is a valid solution for each proposed case, right after that, all equipment specifications are demonstrated. In columns 3 and 4, for both tools, results are convergent with the tool proposed here. Take case number 5 as an example: All solvers were successful in retrieving correctness information from the test case, this is denoted by the word SAT next to the processing time measured in seconds, after this step, the equipment settings (NTP, NBT, Controllers, Inverters and LCC) are shown, in case of equality of the three tools, which we successfully achieved, it can be inferred that the algorithm is soudness, in other words, that the \textbf{EG1} objective was successfully achieved. In case of difference between the acquired results, there is an incongruity in the developed solution. Once we are sure that the proposed system is soudness, we move on to the second scope of this work.
...
%---------------------------------------------------------------------------
\color{red}a 1a parte desta seção está muito parecida com a do VSTTE. Sugiro mudares mais o texto, ou reescrevê-lo, para ficarmos mais distantes do que está lá no outro artigo
\color{blue}We have reported a favorable assessment of the proposed method. Nevertheless, we have also identified five points in two topics to improve and validate in future work. 
\textbf{Electrical System:} (i) Improvement of the power reliability analysis: to
include loss of load probability or loss of power supply
probability, which can make the study more accurate; (ii) Improvement of the system cost analysis by addingoperational and maintenance costs to the adopted LCC analysis; (iii) Increase the equipment and manufacturers database: this will increase the optimization complexity, but the result will also allow improved sizing. 
\textbf{Algorithm: }(iv) The underlying software verifiers employed here perform bitprecise verification based on the Floating-Point (FP) theory. We could use a real arithmetic strategy to tackle these
equations; however, in this study, we have exploited the FP
arithmetic, approximating the real one; (v) Experiment with alternatives for sharing data between processor cores to try to further optimize the tool's response time.

%------------------------------------
\section{Conclusions} 
%------------------------------------
In summary, this work presented an extremely efficient tool, with more accurate results than the HOMER Pro, which has a solid commercial reality. The gains are: providing all the details of each component for a photovoltaic system solution, with complete electrical details, including the manufacturers' data sheet, the component model, rated current and voltage. When we do the analysis regarding the ESBMC and CPAchecker solvers (which are highlighted in their respective areas) versus the proposed tool, the performance issue is considerable, this directly impacts the market, since this work makes it possible to include a solver SMT in traditional market tools with an optimal response time. In addition, we have extended the CEGIS Synthesis Method and implemented this extension within our formal synthesis tool, which allows the optimization of autonomous photovoltaic systems with the best compromise between energy reliability and system cost analysis. For future work we want to make the tool proposed here an API and include it in a commercial tool or develop it on our university campus. Support on-grid systems and optimize calculations for a serious-parallel based system. Finally, we want to apply the knowledge developed in this article to real riverside communities. \color{black}
