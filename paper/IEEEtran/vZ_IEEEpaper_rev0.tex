...
It is worth highlighting some points of attention: (i) PVz considers the average Insolation during the year, this can affect the calculation of the load required to sustain the off-grid system. (ii) The voltage of the home transformers and the energy consumption are very similar, this implies, that the performance results may vary in a more heterogeneous distribution of the energy demand. (iii)
...
We guarantee with the tool developed in this work, the correctness of all test cases, since all three solucinators were able to find the optimal solution, in addition we built the tool in order to process equipment data on a much larger scale, and always focusing on the best solution for the electrical system. Our advances are complemented by the extension of the CEGIS synthesis method implemented in the proposed tool. Finally, we want to apply the knowledge developed in this article to real riverside communities.
