...
\begin{abstract}
Given that the growth of annual global energy consumption is around 1.3\% with forecasts until 2040, photovoltaic systems become a plausible alternative to traditional means of generating energy such as nuclear and fossil. To promote the dissemination of this technology and expand the use of renewable energies, we present the use of formal methods in the development of tools that assist in decision making when implementing an off-grid residential system. Therefore, we evaluated and developed an automated synthesis technique called PVz to obtain the ideal dimensioning of photovoltaic systems with a focus on Life Cycle Cost (LCC) analysis. Given the electrical needs of a home, we seek in a large set of electrical equipment the best possible combination of devices for renewable energy systems, in order to meet the demand required by the house under study, in addition, we calculate all costs related to maintenance such as: equipment replacement, system operation and general maintenance over 20 years. To carry out such optimization, PVz uses a counterexample-guided inductive synthesis approach (CEGIS) to obtain the optimum of each electrical equipment, we understand that our technique is advantageous compared to the tools for photovoltaic dimensioning and optimization available on the market. The results presented here are based on 7 case studies that denote different energy consumptions, some theoretical and others real, these results put the traditional software verifiers face to face, in addition to the commercial tools, we analyze some topics such as solution time, optimal solution , configuration of the resulting systems, among others.
\end{abstract}
