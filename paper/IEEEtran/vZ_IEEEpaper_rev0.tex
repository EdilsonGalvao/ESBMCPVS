...
\textcolor{red} {suitable} alternative to traditional means of generating energy such as nuclear and fossil. To promote the dissemination of this technology and expand the use of renewable energies, we present the use of formal methods in the development of tools that assist in decision making when implementing an off-grid residential system. Therefore, we developed and evaluated an automated synthesis technique called PVz to obtain the %ideal dimensioning 
\textcolor{red} {optimal sizing} of photovoltaic systems with a focus on Life Cycle Cost (LCC) analysis. Given the electrical needs of a home, we seek in a large set of electrical equipment the best possible combination of devices for renewable energy systems, in order to meet the demand required by the house under study, in addition, we calculate all costs related to maintenance such as: equipment replacement, system operation and general maintenance over 20 years. To carry out such optimization, PVz uses a counterexample-guided inductive synthesis approach (CEGIS) to obtain the %optimum of each electrical equipment
\textcolor{red}{technical solution}. We %understand 
clain that our technique is advantageous compared to the tools for photovoltaic sizing and optimization available on the market. The results presented in this paper are based on seven case studies that denote different energy consumptions, some theoretical and others real\textcolor{red}{. These results put some traditional software verifiers face to face. The same case studies were solved by a commercial optimization tool. Our technique and the commercial tool results were both validated with a third commercial design software in order to perform a fair comparison; furthermore} we analyze some topics such as solution time, optimal solution, configuration of the resulting systems, among others.
\end{abstract}
...
The best compromise between two objectives makes the optimal sizing of PV systems: \textit{power reliability} and \textit{system cost}~\cite{Alsadi2018}. This study will rely on the critical period solar energy method~\cite{Pinho}, as described in Section~\ref{sec:sizing}. \color{blue}Our study will use an adapted Life Cycle Cost (LCC) analysis, where the acquisition cost of every item of equipment is considered, plus the installation cost, the operational and maintenance costs~\cite{Alsadi2018}, theses costs are represented by:
\begin{equation}
\label{eq:LCC}
LCC = EC + EM
\end{equation}
