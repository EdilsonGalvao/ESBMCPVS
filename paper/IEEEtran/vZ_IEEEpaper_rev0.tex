...
Algorithm~\ref{base_opt_code} retrieves the information described above based on arguments provided by \textit{solve} function and creates an internal context. \textcolor{red}{This context is responsible for verifying whether all equations are described in algorithm~\ref{base_opt_code2}, specified in steps 04, 06, 11, 13 are satisfactory. in addition, the assumes method is called to ensure that the restrictions described in steps 05, 08, 10 and 12 are met.}

\textcolor{red}{The algorithm contains three essential points. The \textbf{first} point is the set of equations that correspond to a valid combination of electrical components. These equations can be found at steps 02, 03, 04, 06, 07, 09, 11, and 13. The \textbf{second} point refers to all four \textit{assume} method that is necessary since it serves as a limit or barrier of acceptable values to pieces of equipment. These restrictions can be found in steps 05, 08, 10, and 12. The \textbf{third} point, the return system that can be SAT or UNSAT. }
...
\textbf{Reduced approach:} Traditional sofware verifiers, e.g., ESBMC and CPAchecker, face difficulties in solving optimization problems. However, they can be solid allies for detecting the optimal solution. The idea behind this approach is to reduce state-space exploration. \textcolor{red}{The reduction in the state-space was due to the reduction in the number of combinations of electrical equipment, which in the expanded methodology include 93347 arrangements as possible solutions for the photovoltaic system, while in this approach the number of arrangements totals only 24.} \textcolor{red}{In this approach, all verifiers above will solve the proposed photovoltaic systems and compute their respective results. Through reduced methodology , we were able to avoid timeout and memory out problems, thus, we can prove the technique’s efficiency when the results obtained are convergent to same specification, in other words, each test case should have the same result when analyzed by the three SMT solvers.}.
