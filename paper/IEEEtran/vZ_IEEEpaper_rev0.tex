...
\begin{abstract}
Given that the growth of annual global energy consumption is around 1.3\% with forecasts until 2040, photovoltaic systems become a %plausible
\textcolor{black} {suitable} alternative to traditional means of generating energy such as nuclear and fossil. To promote the dissemination of this technology and expand the use of renewable energies, we present the use of formal methods in the development of tools that assist in decision making when implementing an off-grid residential system. Therefore, we developed and evaluated an automated synthesis technique called PVz to obtain the %ideal dimensioning 
\textcolor{black} {optimal sizing} of photovoltaic systems with a focus on Life Cycle Cost (LCC) analysis. Given the electrical needs of a home, we seek in a large set of electrical equipment the best possible combination of devices for renewable energy systems, in order to meet the demand required by the house under study, in addition, we calculate all costs related to maintenance such as: equipment replacement, system operation and general maintenance over 20 years. To carry out such optimization, PVz uses a counterexample-guided inductive synthesis approach (CEGIS) to obtain the %optimum of each electrical equipment
\textcolor{black}{technical solution}. We %understand 
clain that our technique is advantageous compared to the tools for photovoltaic sizing and optimization available on the market. The results presented in this paper are based on seven case studies that denote different energy consumptions, some theoretical and others real\textcolor{black}{. These results put some traditional software verifiers face to face. The same case studies were solved by a commercial optimization tool. Our technique and the commercial tool results were both validated with a third commercial design software in order to perform a fair comparison; furthermore} we analyze some topics such as solution time, optimal solution, configuration of the resulting systems, among others.
\end{abstract}
...
Both modes described here use the same input and theoretically tend to produce the same output, it is noteworthy that variables such as electrification, support for different categories of equipment and different forms of electrical sizing, impact on the expected output.
...
%  Case studies $2$, $3$, $6$, and $7$ led to a \textit{time out} result in CPAchecker, i.e., it was not solved within $10$ hours.  \color{black}
