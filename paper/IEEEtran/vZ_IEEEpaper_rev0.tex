...
\begin{abstract}
Given that annual global energy consumption growth is around 1.3\% with forecasts until 2040, photovoltaic systems become a suitable alternative to traditional means of generating energy such as nuclear and fossil. To promote the dissemination of this technology and expand the use of renewable energies, we present formal methods to develop tools that assist in decision-making when implementing an off-grid residential system. Therefore, we developed and evaluated an automated synthesis technique called PVz to obtain the optimal sizing of photovoltaic systems focusing on Life Cycle Cost (LCC) analysis. Given the electrical needs of a home, we seek a large set of electrical equipment the best possible combination of devices for renewable energy systems to meet the demand required by the house understudy. Also, we calculate all costs related to maintenance, such as equipment replacement, system operation, and general maintenance over 20 years. PVz uses a counterexample-guided inductive synthesis approach (CEGIS) to obtain the technical solution to carry out such optimization. We claim that our technique is advantageous compared to the tools for photovoltaic sizing and optimization available on the market. The results presented in this paper are based on seven case studies that denote different energy consumptions, some theoretical and others real. These results put some traditional software verifiers face to face. The same case studies were solved by a commercial optimization tool. Our technique and the commercial tool results were validated with another commercial design software to perform a fair comparison; furthermore, we analyze some topics such as solution time, optimal solution, and configuration of the resulting systems.
\end{abstract}
...
Research in the fields of renewable energy and photovoltaic systems, date from the mid-90s, and bring different approaches to find an optimal solution for the design of electrical systems as can be seen in~\cite{Driouich2018} and~\cite{Applasamy2011}. However, the amount of work-related to formal modeling of photovoltaic systems is still low. Consequently, it is even more challenging to find work based on modern solvers. The starting point for this work~\cite{VSTTE2020}~\cite{TrindadeCordeiro19}, are researches that derive from the year 2019 and converge to analyze the optimization problem of photovoltaic systems using SMT solvers and modern commercial tools such as HOMER and PVsyst. In other words, this paper makes the following original contributions. First, it is a radical improvement in the base paper's performance describing formal synthesis for stand-alone PV systems application. Second, the technique described here supports data processing parallelization since it relies on a multi-core processor. Third, the state space coverage was increased compared to the base article~\cite{VSTTE2020}. It was possible to cover and compute all the spaces of the combinations of electrical equipment. Fourth, experimental results with seven case studies show that the formal synthesis approach qualitatively outperforms an existing state-of-the-art optimization tool. Lastly, the results are validated with accurate commercial design software called PVsyst.

%\textit{Contribution}. Our study describes the first application of a sound and automated formal synthesis approach to provide accurate results of optimal sizing of stand-alone PV systems. It outperforms an existing state-of-the-art simulation tool used in comparison with seven case studies. In particular, the methodological research with automated synthesis produces more detailed and complete optimization when compared with the simulation tool. The sizing is closer to real PV systems deployed at isolated communities in the Amazon area of Brazil. Considering other studies in the last two decades, we considerably advanced state-of-the-art applied energy to design PV systems optimally.
...
Algorithm~\ref{base_opt_code} is started based in input described on Fig.~\ref{fig:optimization}, it contemplate manufacturer’s data, prices of PV panels, batteries, charge controllers, and inverters. Programmatically, this set of data is represented in arrays and global variables, which can assist the storage of counterexamples and the maximum possible cost \textcolor{red}{could you please point to the lines of the algorithm?}. Moreover, we define design (house) requirements and design assumptions to help backend solvers. 
...
The node representation stores the lower and upper limits, the counterexample, and other auxiliary variables. The parallel loop is used to process each node separately. The function \textit{solve} described in Algorithm~\ref{base_opt_code2} returns true when the node is a possible solution, and the cost is lower than the global maximum cost. Therefore, to assure the correct storage of results processed by many threads simultaneously, the lock function was added. After the internal loop ends, the last action returns the optimal solution found in the list that contains all possible solutions.
...
%---------------------------------------------------------------------------
 ...
 Table~\ref{Tab:Tcr} condenses the result of both approaches described in section~\ref{sec:SolversandStateSpace}. The table is divided into four important sets of columns, read from left to right: Column 01 describes all test cases with their respective specifications. Columns 02, 03, and 04 refer to the reduced approach that proves the system's correctness. Columns 05, 06, and 07 refer to the expanded approach, which proves the robustness of the tool used for commercial analysis. Finally, the last column refers to the simulation of the photovoltaic system using HOMER Pro.

\textcolor{red}{This sentence is broken... please rewrite it.} \textbf{Reduced approach:} In view of the state explosion problem mentioned in Section~\ref{sec:SolversandStateSpace}, which demands enormous time and computational power from the ESBMC tool and the CPAchecker tool, it was necessary to choose the smallest possible amount of data that would satisfy the computational calculations to find the better configuration of electrical equipment for the photovoltaic system. To reach the goal of 70 different devices described in section~\ref{sec:SynthesizingOptimalSolarPhotovoltaicSystems}, we need to process a smaller set of data in this approach totaling only ten devices divided into four different categories and represented in Figure~\ref{fig:blockdiagram}. This change implies reducing the sample processing space by approximately 3890 times. Consequently, the answer time will be reduced.
...
Once it has been confirmed that the EG1 objective has been achieved, we must note that the tools' crucial difference is the processing time. PVC obtained interesting results, being approximately 13 times faster when compared to ESBMC and hundreds of thousands of times faster than CPAchecker. Our reduced approach solution proved to be robust against two of the best SMT solvers.

\textcolor{red}{This sentence is broken... please rewrite it.} \textbf{Expanded approach:} With the growing global market towards investments in photovoltaic technologies, it is expected that new equipment will be developed, this aspect forces the tool proposed here to support the addition of new equipment on a large scale. In opposition to the first approach, we radically increased the amount of equipment and, as a result, we increased the possible combinations of solutions exponentially. Therefore, this approach ensures that the EG02 and EG03 objectives can be achieved. Initially, it must be proved that the performance of the tool proposed here has gained superior to the traditional tools (ESBMC / CPAchecker) in the item of processed data and in the amount of time spent on processing the solution. Besides, the proposed tool should have competitive gains concerning market tools such as HOMER Pro.
...
Concerning the performance, the difference between each test case is less than two minutes on average. HOMER Pro executed the cases on average three times faster. The $\nu$Z takes a little more time; however, it returns the optimal cost, HOMER Pro returns an average cost of around 5\% more or less than the correct value. \textcolor{red}{This sentence is unclear...} It is possible to find theses values, removing the test case with maximum and minimum values, after it, removing the test case 3 that HOMER Pro does not resolved, finally, calculating the average price (LCC) of the set exposed in this work, we obtain the value around US\$ 450 dollars.
...
\textcolor{red}{This section should not be about future work but the threats to our experiments' validity.}
  It is worth highlighting some points for improvement to future works. (i) Improve the technique described in this work to increase the quantity of equipment (in the hundreds of hundreds) without significant loss of performance. (ii) Include an analysis of pressure loss or energy loss in the power supply, which helps prevent real systems' electrical surges. (iii) Converting the equations encoded in fluent points to real arithmetic theory.

%------------------------------------
\section{Conclusions} 
%------------------------------------

\textcolor{red}{This sentence is too long and very unclear...} In summary, the tool presented here is extremely efficient, bringing results with greater precision (providing all the details of each component of a photovoltaic system, with complete electrical details, including the manufacturer's datasheet, the component model, rated current and voltage) and more performance when compared to ESBMC and CPAchecker, which are state of the art in SMT solvers. PVC was shown to be more efficient in both approaches described in section ~\ref{sec:SolversandStateSpace}, was dozens of times faster than the best case (ESBMC) in the first approach, and was able to process a significantly greater number of states and equipment combinations without exceeding the memory limit available for testing in the second approach, which unfortunately traditional SMT solvers have failed. When we talk about a commercial tool, HOMER tends to be a great choice; however, one must consider all the restrictions mentioned in section~\ref{sec:SimulationToolsandAssumptions}. When comparing PVz with HOMER, it is evident that state coverage and proximity to performance make choosing PVz a valuable and less expensive alternative. Our advances are further complemented by the extension of the CEGIS synthesis method implemented in the proposed tool. We want to make the tool proposed here an API and include it in a commercial tool or develop it on our university campus for future work. Finally, we want to apply the knowledge developed in this article to real riverside communities.
