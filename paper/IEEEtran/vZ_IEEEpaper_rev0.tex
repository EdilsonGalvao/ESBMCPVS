...
\section{Conclusions} 
%------------------------------------
In summary, this work presented an extremely efficient tool, with more accurate results than the Homer Pro, which has a solid commercial reality. The gains are: providing all the details of each component for a photovoltaic system solution, with complete electrical details, including the manufacturers' data sheet, the component model, rated current and voltage. When we do the analysis regarding the ESBMC and CPAchecker solvers (which are highlighted in their respective areas) versus the proposed tool, the performance issue is considerable, this directly impacts the market, since this work makes it possible to include a solver SMT in traditional market tools with an optimal response time.

In addition, we have extended the CEGIS Synthesis Method and implemented this extension within our formal synthesis tool, which allows the optimization of autonomous photovoltaic systems with the best compromise between energy reliability and system cost analysis.

For future work we want to make the tool proposed here an API and include it in a commercial tool or develop it on our university campus. Support on-grid systems and optimize calculations for a serious-parallel based system. Finally, we want to apply the knowledge developed in this article to real riverside communities. \color{black}
