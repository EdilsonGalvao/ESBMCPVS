...
PVz proved to be an extremely efficient tool, solving photovoltaic systems with greater precision and resolution time very close to that of market tools such as HOMER Pro \textcolor{red}{what do you mean by ``solving photovoltaic systems''?}. PVz withstood an exhaustive data load and concluded all benchmarks by delivering an optimal solution. The same has not happened with ESBMC and CPAchecker, which are state-of-the-art in software verification.
