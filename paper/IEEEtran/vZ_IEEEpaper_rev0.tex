...
Algorithm~\ref{base_opt_code} \textcolor{red}{is started based in input  data described on Fig.~\ref{fig:optimization}, programmatically this content is represented in matrices way and in global variables, the first line contemplate manufacturer’s data, prices of PV panels, batteries, charge controllers and inverters. Lines 2, 3 and 4 represent respectively, the state-space, the nodes that satisfy the equations and the lowest global cost. Moreover, we define design (house) requirements and design assumptions to help backend solvers.}
...
\textbf{Reduced approach:} \textcolor{red}{Traditional tools such as ESBMC and CPAchecker have severe difficulties in solving optimization problems, however, they can be strong allies for detecting the optimal solution. The idea behind this approach is to reduce the state-space in such a way that the solution of all the test cases is contained in the reduced set of solutions, the reduction of the state-space is around 3890 times. In this approach, all the aforementioned solvers will be able to solve the proposed photovoltaic systems and compute their respective results. Through this methodology we were able to avoid timeout and memory out problems, so we can prove the efficiency of the technique when the results obtained are coverage.}
...
\textbf{Expanded approach:} \textcolor{red}{Once we have the solutions of the test cases found by the three aforementioned solvers, we must turn our eyes to the objectives EG02 and EG03 respectively. In opposition to the first approach, the amount of equipment has increased considerably, this implies an increase in the computational cost to find the optimal solution.}

\textcolor{red}{For objective EG02, we analyzed the performance of the proposed technique, versus the ESBMC, CPAchecker and HOMER Pro tools. In table~\ref{Tab:Tcr}, it is possible to infer that both the ESBMC and CPAchecker tools failed to try to find a feasible solution for all the proposed simulations. Homer Pro, obtained all the results with the exception of the third case described in table~\ref{Tab:Tcr} column 08. For objective EG03, we make a comparison between columns 05 and 08, we conclude that the technique we have implemented is more accurate in relation to the combinations of electrical equipment and their respective prices and LCC, while, HOMER Pro which we will describe in more detail in the next paragraph, processes the system a few seconds faster with an acceptable answer, but that is not the optimal one.}
...
\textcolor{red}{Regarding the processing time, both solved all the case studies in less than two minutes, which shows a significant advance in relation to the other tools mentioned here. HOMER Pro solves the seven test cases 3 times faster than PVz, in counterpart to the performance, HOMER Pro does not return the global optimum cost, just an approximation, in our scenario we find that this discrepancy is around 5\% ( financially this variation is US \$ 450 dollars) upwards or downwards. To arrive at these numbers, we took the seven test cases, so we removed the third test case since HOMER Pro did not find a solution, right after, the average price (LCC) was made, removing the test case with higher cost and lower cost.}
...
%---------------------------------------------------------------------------
\textcolor{red}{
It is worth highlighting some points of attention: (i) To increase the accuracy of the technique, it is necessary to increase the equipment database, however, this directly affects the complexity of computational processing. (ii) All costs (LCC) are based on a local reality, it is necessary to adapt the inputs to support solar radiation, annual maintenance costs and equipment costs from other countries and cities. (iii) The factors that determine the energy demand of the test cases are constant elements in the technique described here, and ignore the different seasons and changes in solar radiation. (IV) Finally, the results described in the test cases must be implemented electrically under real conditions of use.}

%------------------------------------
\section{Conclusions} 
%------------------------------------
\textcolor{red}{PVz proved to be an extremely efficient tool, solving photovoltaic systems with greater precision and with a resolution time very close to that of market tools such as HOMER Pro. PVz withstood an exhaustive data load and concluded all test cases by delivering an optimal solution, the same has not happened with ESBMC and CPAchecker, which are the state of the art in SMT solvers.}

\textcolor{red}{We guarantee with the tool developed in this work, the correctness of all test cases, since all three solucinators were able to find the optimal solution, in addition we built the tool in order to process equipment data on a much larger scale, and always focusing on the best solution for the electrical system. Our advances are complemented by the extension of the CEGIS synthesis method implemented in the proposed tool. Finally, we want to apply the knowledge developed in this article to real riverside communities.}
