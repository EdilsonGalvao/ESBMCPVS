...
float Insol = 3.80; float LCC;

/*
 * these properties represent electrical specifications for each project.
 */
int Phouse = MAX_INTEGER, Psurge = MAX_INTEGER, Econsumption = MAX_INTEGER; VAC = MAX_INTEGER;
            ...
            for (int c = 0; c < common_size; c++)
            {
                for (int i = 0; i < common_size; i++)
                {
                    ...
                    if (cost < LOWEST)
                    {
                        ...
                        //printf("Counter:%i P:%i B:%i C:%i I:%i => Cost:%d\n", count, p, b, c, i, LOWEST);
                    }
                }
            }
        }
